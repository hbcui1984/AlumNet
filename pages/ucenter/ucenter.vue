<template>
	<view class="center">
		...
		<!-- 已认证：显示迷你校友卡 -->
		<view v-if="isVerified && cardData" class="mini-card" :style="{ background: cardGradient }" @click="goToCard">
			<view class="mini-card-left">
				<text class="mini-card-name">{{ cardData.realName }}</text>
				<text class="mini-card-meta">{{ cardData.schoolName }}</text>
				<text class="mini-card-meta">{{ formatGraduationYear }}</text>
				<view v-if="cardData.college" class="mini-card-tag-row">
					<text class="mini-card-tag">{{ cardData.college }}</text>
					<text v-if="cardData.major" class="mini-card-tag">{{ cardData.major }}</text>
				</view>
				<text class="mini-card-no">No. {{ cardData.alumniCardNo }}</text>
			</view>
			<image
				:src="cardData.cardPhotoUrl || cardData.avatar || '/static/default-avatar.png'"
		...
		<!-- 未认证或未登录：普通头像区域 -->
		<view v-else class="userInfo" @click.capture="toUserInfo">
			<view class="userInfo-inner">
				<cloud-image v-if="hasLogin&&userInfo.avatar_file&&userInfo.avatar_file.url" :src="userInfo.avatar_file.url" class="avatar"></cloud-image>
				<image v-else-if="hasLogin&&userInfo.avatar" :src="userInfo.avatar" class="avatar" mode="aspectFill"></image>
				...
				<uni-icons type="arrowright" size="18" color="rgba(255,255,255,0.6)"></uni-icons>
			</view>
		</view>
...
<script>
	...
	export default {
		...
		onShow() {
			if (this.hasLogin && this.userInfo.alumniStatus === 1) {
				this.loadCardData()
			}
		},
		computed: {
			...
			async loadCardData() {
				try {
					...
					const [cardRes, configRes] = await Promise.all([
						alumniObj.getAlumniCard(),
						alumniObj.getSchoolConfig()
					])
					if (cardRes.errCode === 0) this.cardData = cardRes.data
					if (configRes.errCode === 0) this.schoolConfig = configRes.data
				} catch (e) {
					console.error('加载校友卡失败', e)
				}
			},
			goToCard() {
				uni.navigateTo({ url: '/pages/alumni/card/card' })
			},
			toSettings() {
				uni.navigateTo({
